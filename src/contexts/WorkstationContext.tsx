...
export interface WorkstationContextType {
  ...
  getTrackCurrentValue?: (track: Track) => {
    value: number;
    isAutomated: boolean;
  };
  insertClips: (
  ...
  updateTimelineSettings: (
    updater: (prev: TimelineSettings) => TimelineSettings
  ) => void;
...
export const WorkstationProvider: React.FC<WorkstationProviderProps> = ({
  ...
  const [maxPos] = useState<TimelinePosition>(new TimelinePosition(32, 0, 0)); // setMaxPos removed as unused
  ...
  const [snapGridSize] = useState<TimelinePosition>(
  ...
  const addNode = useCallback(
    (track: Track, lane: AutomationLane, node: AutomationNode) => {
      const updatedTrack = { ...track };
      const laneIndex = updatedTrack.automationLanes.findIndex(
        (l) => l.id === lane.id
      );
      if (laneIndex > -1) {
        updatedTrack.automationLanes[laneIndex] = {
          ...lane,
          nodes: [...(lane.nodes || []), node].sort((a, b) =>
  ...
  const pasteNode = useCallback(
    (/* pos: TimelinePosition, lane: AutomationLane */) => {
  ...
  const setLane = useCallback(
    (track: Track, lane: AutomationLane) => {
      const updatedTrack = { ...track };
      const laneIndex = updatedTrack.automationLanes.findIndex(
        (l) => l.id === lane.id
      );
  ...
  const value: WorkstationContextType = {
    ...
    createAudioClip:
      async (/* file: File | Blob, position?: TimelinePosition */) => {
