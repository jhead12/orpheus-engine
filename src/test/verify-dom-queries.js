...
function testPeakDisplay() {
  try {
    ...
    console.log(
      `✅ Found ${peakByClass.length} peak displays using class selector`,
    );
    ...
    console.log(
      `✅ Found peak display through parent: ${peakInMeter ? 'Yes' : 'No'}`,
    );
...
function testKnob() {
  try {
    ...
    console.log(
      `✅ Found ${knobByTestId.length} knobs using data-testid selector`,
    );
    console.log(
      `✅ Title attribute: "${knobByTestId[0]?.getAttribute('title')}"`,
    );
