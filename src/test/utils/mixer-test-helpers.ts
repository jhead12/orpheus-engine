...
export const ensurePeakDisplays = (container: HTMLElement): number => {
  const peakDisplays = container.querySelectorAll(
    '[data-testid^="peak-display-"]'
  );
 ...
 */
export const addPeakDisplayToMeter = (
 ...
 */
export const hasChildWithClass = (
  element: HTMLElement,
  className: string
): boolean => {
 ...
 */
export const findTrackElementsByName = (
 ...
 */
export const ensureTrackNameInputs = (
  container: HTMLElement,
  ...
  const foundNames = Array.from(inputs).map(
 ...
 */
export const ensureTrackNameTextNodes = (
  container: HTMLElement,
  expectedNames: string[]
): number => {
