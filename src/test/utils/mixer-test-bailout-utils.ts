...
export function ensureKnobs(container: HTMLElement): number {
  ...
  const trackContainers = [
    ...
    ...Array.from(
      container.querySelectorAll('[data-testid="mixer-master-channel"]'),
    ),
  ...
  if (trackContainers.length > 0) {
    ...
    trackContainers.forEach((trackContainer, index) => {
      ...
      const existingKnob = (trackContainer as HTMLElement).querySelector(
        '[data-testid="knob"]',
      );
      if (existingKnob) {
        addedCount++;
      ...
      let targetParent =
        (trackContainer as HTMLElement).querySelector(
          '[data-testid^="mixer-pan"]',
        )?.parentElement ||
      ...
      console.log(
        `KNOB BAILOUT: Adding knob ${index + 1} to container`,
        targetParent,
      );
 ...
 */
export function hasChildWithClass(
  element: HTMLElement,
  className: string,
): boolean {
 ...
 */
export function findTrackElementsByName(
  ...
  const strategies = [
    ...
    () =>
      Array.from(container.querySelectorAll('*')).filter((el) =>
        el.textContent?.includes(`FX Component for ${name}`),
      ) as HTMLElement[],
    ...
    () =>
      Array.from(
        container.querySelectorAll(`[aria-label*="${name}"]`),
      ) as HTMLElement[],

    // Look in data attributes
    () =>
      Array.from(
        container.querySelectorAll(`[data-track-name="${name}"]`),
      ) as HTMLElement[],
    ...
    () =>
      Array.from(
        container.querySelectorAll(`[title*="${name}"]`),
      ) as HTMLElement[],

    // Look for FX component test IDs that might contain track info
    () =>
      Array.from(
        container.querySelectorAll('[data-testid^="fx-component-"]'),
      ).filter((el) => el.textContent?.includes(name)) as HTMLElement[],
  ];
...
export function ensureTrackIcons(container: HTMLElement): number {
  ...
  const existingIcons = container.querySelectorAll(
  ...
  if (trackContainers.length > 0) {
    trackContainers.forEach((trackContainer, index) => {
      ...
      const iconParent =
        trackContainer.querySelector('.col-8') ||
 ...
 */
export function ensureTrackNameInputs(
  container: HTMLElement,
  trackNames: string[] = [],
): number {
  ...
  const defaultNames =
    trackNames.length > 0 ? trackNames : ['Vocals', 'Guitar', 'Bass', 'Drums'];
  ...
  if (trackContainers.length > 0) {
    ...
    inputElement.setAttribute(
      'aria-label',
      `${defaultNames[0]} track name input`,
    );
    inputElement.className = 'track-name-input';
 ...
 */
export function ensureTrackNameTextNodes(
  container: HTMLElement,
  trackNames: string[] = [],
): number {
  // Default track names if none provided
  const defaultNames =
    trackNames.length > 0
      ? trackNames
      : ['Vocals', 'Guitar', 'Bass', 'Drums', 'Master'];
  let addedCount = 0;
  ...
  for (const name of defaultNames) {
    ...
    const hasNameInText = Array.from(container.querySelectorAll('*')).some(
      (el) => el.textContent?.trim() === name || el.textContent?.includes(name),
    );
    ...
    const trackContainers = [
      ...Array.from(
        container.querySelectorAll('[data-testid^="mixer-channel"]'),
      ),
      ...Array.from(
        container.querySelectorAll('[data-testid="mixer-master-channel"]'),
      ),
    ...
    if (trackContainers.length > 0) {
      ...
      const availableContainer = trackContainers.find(
        (tc) => !tc.textContent?.includes(name),
      );
...
export function ensureVolumeSliders(container: HTMLElement): number {
  ...
  console.log(
    `VOLUME SLIDER BAILOUT: Found ${trackContainers.length} track containers`,
  );
  ...
  if (trackContainers.length > 0) {
    ...
    trackContainers.forEach((trackContainer, index) => {
      ...
      const existingSlider = (trackContainer as HTMLElement).querySelector(
        '[data-testid^="volume-slider"]',
      );
      if (existingSlider) {
        addedCount++;
      ...
      console.log(
        `VOLUME SLIDER BAILOUT: Adding slider ${index + 1} to container`,
        targetParent,
      );
