...
function generateImports(data: VisualTestTemplateData): string {
  ...
  lines.push(
    `import { ${data.ComponentName} } from "../${data.ComponentPath}";`
  );
  return joinLines(lines);
}

function generateInteraction(
  interaction: NonNullable<VisualTestState['interactions']>[number]
): string {
  const valueArg = interaction.value
...
export function generateVisualTest(data: VisualTestTemplateData): string {
  ...
  const testCases = data.states
    .map((state) => generateTestCase(data, state))
    .join('\n\n');
