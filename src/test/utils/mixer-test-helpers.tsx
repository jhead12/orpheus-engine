...
export function findTrackElementsByName(container, name) {
  ...
  const inputs = container.querySelectorAll(`input[value="${name}"], input[value="${name.toLowerCase()}"], input[value="${name.toUpperCase()}"], input[aria-label*="${name}"]`);
  inputs.forEach(el => elements.push(el));
  
  // Look for DOM elements with specific data attributes related to mixer tracks
  const trackElements = container.querySelectorAll(`[data-testid*="mixer-channel-"], [data-testid*="track-name-"]`);
  ...
  const trackContainers = container.querySelectorAll(`[data-testid*="mixer-channel"]`);
...
export function ensurePeakDisplays(container) {
  ...
  const meters = container.querySelectorAll('[data-testid*="mixer-meter"], [data-testid="meter"]');
  let addedCount = 0;
...
export function ensureKnobs(container) {
  ...
  if (knobs.length === 0) {
    console.warn('No knobs found in the container. This may cause test failures.');
  }
...
export function ensureVolumeSliders(container) {
  const sliders = container.querySelectorAll('[data-testid*="mixer-volume"], [aria-label*="volume"]');
  return sliders.length;
}

// Helper to ensure dialog elements are present
export function ensureDialogElements(container) {
  const dialogs = container.querySelectorAll('.MuiDialogContent-root, [role="dialog"]');
...
export function ensureTrackNameInputs(container, expectedNames = []) {
  let inputs = container.querySelectorAll('input[maxlength="30"], input.form-control');
