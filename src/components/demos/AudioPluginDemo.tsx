...
export const AudioPluginDemo: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initializeSystem = async () => {
      try {
        ...
        try {
          const backend = pluginSystem.getRegistry().getPlugin('orpheus.audio.analysis')?.plugin as any;
          if (backend?.context?.pythonBackend) {
            const isConnected = backend.context.pythonBackend.isBackendConnected();
    ...
    for (const file of files) {
      try {
        ...
        if (analysisData) {
          ...
          const analysisPlugin = pluginSystem.getActivePlugins().find(
  ...
  const renderPluginUI = (plugin: any) => {
    if (plugin.manifest.id === 'orpheus.audio.analysis' && plugin.plugin.createUI) {
    ...
    <div className="audio-plugin-demo">
      <div className="demo-header">
        ...
        <div className="status-indicators">
          <div className={`status-indicator ${pluginSystemReady ? 'connected' : 'disconnected'}`}>
          ...
          <div className="audio-files-list">
            {loadedFiles.length === 0 ? (
              <div className="empty-state">
                <p>No audio files loaded. Click "Load Audio Files" to get started.</p>
                ...
                <div key={file.id} className="audio-file-item">
                  <div className="file-info">
                    ...
                    <div className="file-details">
                      {file.duration.toFixed(2)}s • {file.sampleRate}Hz • {file.channels} ch
                    </div>
                  </div>
        ...
        <div className="plugins-section">
          ...
          <div className="plugins-list">
            {activePlugins.length === 0 ? (
              <div className="empty-state">
                <p>No plugins active. Enable plugins to see analysis results.</p>
              </div>
            ) : (
                ...
                <div key={plugin.manifest.id} className="plugin-item">
                  <div className="plugin-header">
                    <div className="plugin-info">
                      ...
                      <div className="plugin-category">{plugin.manifest.category}</div>
                    </div>
      ...
      <div className="demo-footer">
        <p>
          This demo showcases the Orpheus Engine plugin architecture with real-time audio analysis.
          Load audio files to see automatic analysis results from AI-powered plugins.
        </p>
      </div>
    </div>
  );
