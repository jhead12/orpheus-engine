...
>(props: IProps<T>) {
  ...
  function next() {
    if (selectedIdx === -1) onChange(options[options.length - 1].value);
    else if (selectedIdx < options.length - 1)
      onChange(options[selectedIdx + 1].value);
  }

  function prev() {
    if (selectedIdx === -1) onChange(options[0].value);
    else if (selectedIdx > 0) onChange(options[selectedIdx - 1].value);
  }
  ...
  return (
    ...
    >
      ...
      <div
        ...
        onClick={() => {
          if (!disableSelect) setAnchorEl(ref.current);
        }}
        style={style.select}
      >
        {selectedIdx === -1 ? (
