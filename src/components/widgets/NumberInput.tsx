...
export default function NumberInput(props: IProps) {
    ...
    function handleMouseMove(e: MouseEvent) {
      if (dragStartValue) {
        ...
        if (movementY === 0 && Math.abs(e.movementX) < 3) {
          if (e.screenY <= 0) movementY = -1;
          else if (e.screenY >= window.screen.height - 1) movementY = 1;
        }

        yDeltaFromStart.current += movementY;

        let newValue =
          dragStartValue - Math.ceil(yDeltaFromStart.current / 3) * step;

        if (e.movementY < 0)
          newValue = start + step * Math.floor((newValue - start) / step);
        else newValue = start + step * Math.ceil((newValue - start) / step);

        props.onChange(
          clamp(newValue, props.min ?? -Infinity, props.max ?? Infinity)
        );
      }
    }
  ...
  function decrement() {
    if (!props.disabled) {
      ...
      let newValue =
        ref.current === document.activeElement ? getTextValue() : props.value;
      newValue -= step;
  ...
  function getSpinButtonIconProps(
    decrement: boolean,
    icon?: 'arrow' | 'plusminus'
  ) {
    return {
  ...
  function handleChange(e: React.ChangeEvent<HTMLInputElement>) {
    if (
      ...
      if (
        ...
        !newCharacters.current.includes('.') ||
        !props.integersOnly
      )
  ...
  function handleKeyDown(e: React.KeyboardEvent) {
    if (e.key.length === 1) newCharacters.current = e.key;
    else if (e.key === 'Backspace' || e.key === 'Delete')
      newCharacters.current = -1;
    else if (e.key === 'ArrowUp') increment();
  ...
  const layout1SpinButtons = (
    ...
    >
      ...
      >
        <SpinButtonIcon
          {...getSpinButtonIconProps(false, props.buttons?.icon || 'arrow')}
        />
      </HoldActionButton>
      <HoldActionButton
      ...
      >
        <SpinButtonIcon
          {...getSpinButtonIconProps(true, props.buttons?.icon || 'arrow')}
        />
      </HoldActionButton>
    </div>
  );
  ...
  const layout2IncreaseButton = (
    ...
    >
      <SpinButtonIcon
        {...getSpinButtonIconProps(false, props.buttons?.icon || 'plusminus')}
      />
    </HoldActionButton>
  );
  ...
  const layout2DecreaseButton = (
    ...
    >
      <SpinButtonIcon
        {...getSpinButtonIconProps(true, props.buttons?.icon || 'plusminus')}
      />
    </HoldActionButton>
  );
  ...
  const startButton =
    props.orientation !== 'vertical'
      ? layout2DecreaseButton
      : layout2IncreaseButton;
  const endButton =
    props.orientation !== 'vertical'
      ? layout2IncreaseButton
      : layout2DecreaseButton;
  ...
  return (
    ...
    >
      {props.layout === 'alt' &&
        props.buttons?.show !== false &&
        (props.reverseOrientation ? endButton : startButton)}
      {props.layout !== 'alt' &&
        props.buttons?.show !== false &&
        props.reverseOrientation &&
        layout1SpinButtons}
      <form
      ...
      >
        <input
          ...
          onFocus={() => {
            if (!inputDisabled) setDisableClickAndDrag(true);
          }}
          onKeyDown={handleKeyDown}
          onPaste={(e) =>
            (newCharacters.current = e.clipboardData?.getData('text'))
          }
          ...
          style={{
            ...
            backgroundColor: '#0000',
            pointerEvents:
              props.clickAndDrag && !disableClickAndDrag ? 'none' : 'auto',
      ...
      {props.layout === 'alt' &&
        props.buttons?.show !== false &&
        (props.reverseOrientation ? startButton : endButton)}
    </div>
  );
}
