...
export interface TooltipProps {
  ...
  container?:
    | TooltipContainer
    | { horizontal?: TooltipContainer; vertical?: TooltipContainer };
  open?: boolean;
  showOnHover?: boolean;
...
export default function Tooltip(props: TooltipProps) {
  ...
  const containerElements = useRef({
  ...
  function updatePosition() {
    ...
    if ((open || (showOnHover && hovering)) && anchor && tooltipEl) {
      ...
      const hContainerRect =
        containerElements.current.horizontal.getBoundingClientRect();
      const hPlacement = placement?.horizontal || 'center';
      const vContainerRect =
        containerElements.current.vertical.getBoundingClientRect();
      ...
      else if (vPlacement === 'bottom') top = rect.top + rect.height + 7;
      ...
      if (top - topBound < 7)
        ...
        top =
          (vPlacement === 'center'
            ? bottomBound - tooltipEl.clientHeight
            : rect.top - tooltipEl.clientHeight) - 7;

      if (hPlacement === 'left') left = rect.left - tooltipEl.clientWidth - 7;
      else if (hPlacement === 'right') left = rect.left + rect.width + 7;
      ...
      if (left - leftBound < 7)
        ...
        left =
          (hPlacement === 'center'
            ? rightBound - tooltipEl.clientWidth
  ...
  return (
    <React.Fragment>
      ...
      {(open || (showOnHover && hovering)) &&
        createPortal(
          <div className="px-1" ref={tooltipRef} style={tooltipStyle}>
            {title}
          </div>,
