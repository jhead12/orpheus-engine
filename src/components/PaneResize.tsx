...
export default function PaneResize(props: IProps) {
  ...
  const [activePaneData, setActivePaneData] = useState({
    idx: -1,
    nextIdx: -1,
    availableSize: 0,
  });
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < inputPanes.length; i++) {
      ...
      const previous = prevInputPanes.find(
        (pane) => pane.key === inputPanes[i].key
      );
      ...
      if (
        inputPanes[i].size !== undefined &&
        (!previous || previous.size !== inputPanes[i].size)
      ) {
      ...
      if (existingPane || !pane.auto) availableNewAutoSize -= size;
      else numNewAutoPanes++;

      newPanes.push(pane);
    }

    for (let i = 0; i < panes.length; i++) oldTotalSize += panes[i].size;
    ...
    for (let i = 0; i < newPanes.length; i++) {
      if (panes.find((pane) => pane.key === newPanes[i].key)) {
        if (canStretch[i]) totalUnchangedExistingSize += newPanes[i].size;
        else availableUnchangedExistingSize -= newPanes[i].size;
      } else {
        if (newPanes[i].auto) {
          ...
          else newPanes[i].size = availableNewAutoSize / numNewAutoPanes;

          if (newPanes[i].size < min) newPanes[i].size = min;
          if (newPanes[i].size > max) newPanes[i].size = max;
        }
      ...
      if (
        ...
        if (panes.find((pane) => pane.key === newPanes[i].key)) {
          if (canStretch[i]) {
            ...
            newPanes[i].size *=
              availableUnchangedExistingSize / totalUnchangedExistingSize;
    ...
    stretchPanes(
      newPanes.filter((pane, idx) => {
        if (
          panes.find((p) => p.key === pane.key) &&
          oldTotalSize < containerSize &&
    ...
    function handleResize(e: MouseEvent) {
      ...
      if (idx > -1) {
        ...
        let newSize = Math.min(
          max,
          Math.max(min, tempNewSize, 0),
          availableSize
        );

        if (nextIdx > -1) {
          if (
            newPanes[idx].size + newPanes[nextIdx].size + 1 >=
            availableSize
          ) {
            const nextMin = getAbsoluteSize(newPanes[nextIdx].min ?? 10)!;
            const nextMax = getAbsoluteSize(
              newPanes[nextIdx].max ?? availableSize
            )!;
            ...
            newTempPanes[nextIdx] = {
              ...newTempPanes[nextIdx],
              size: availableSize - tempNewSize,
            };
            newPanes[nextIdx] = {
              ...newPanes[nextIdx],
              size: availableSize - newSize,
            };
          }
        }
    ...
    if (entries.length > 0) {
      ...
      if (
        ...
        if (prevContainerSize.current > 0) {
          ...
          if (
            !(
              totalPaneSize < prevContainerSize.current - 1 &&
              totalPaneSize < containerSize
            )
          ) {
            let panesToStretch = newPanes.filter((pane) => {
              if (!pane.fixed) {
                if (containerSize < totalPaneSize) {
                  if (pane.size > getAbsoluteSize(pane.min ?? 10)!) return true;
                } else {
  ...
  function stretchPanes(panes: Pane[], value: number) {
    ...
    while (remaining > Number.EPSILON) {
      const numPanesToStretch = panes.filter((pane) => {
        if (value > 0) return pane.size < getAbsoluteSize(pane.max ?? Infinity);
        else return pane.size > getAbsoluteSize(pane.min ?? 10);
      }).length;

      if (numPanesToStretch === 0) break;
  ...
  return (
    ...
    >
      {panes.map((pane, index) => {
        ...
        const { auto, children, fixed, handle, key, min, max, size, ...rest } =
          pane;
        const style =
          direction === 'vertical'
