...
import { BaseClipComponentProps } from '@orpheus/types/components';
...
import {
  ...
  audioContext,
  reverseAudio,
} from '@orpheus/services/utils/audio';
...
function AudioClipWaveform({
  ...
  useLayoutEffect(() => {
    if (copyFrom && canvasRef.current) {
      ...
      if (ctx) ctx.drawImage(copyFrom.canvas, 0, 0, width, height);
    }
  }, [copyFrom]);

  return (
    <div
      className="position-relative d-flex pe-none flex-column overflow-hidden"
      style={{ height }}
    >
      {copyFrom ? (
        <canvas
...
function AudioClipComponent({
  ...
  const [waveformLevelsOfDetail, setWaveformLevelsOfDetail] =
    useState<WaveformLODs | null>(null);
  ...
  const url = useMemo(() => {
    return URL.createObjectURL(
      new Blob([clipAudio.buffer], { type: clipAudio.type })
    );
  }, [clipAudio.buffer]);

  useEffect(() => {
    if (audioWidth > WAVEFORM_CHUNK_SIZE) setCopyFrom(undefined);
  }, [audioWidth]);

  const waveformData = useMemo(() => {
    if (waveformLevelsOfDetail) {
      const samplesPerPixel =
        waveformLevelsOfDetail.high[0].length / audioWidth;

      if (samplesPerPixel > 250) return waveformLevelsOfDetail.ultraLow;
      else if (samplesPerPixel > 100) return waveformLevelsOfDetail.low;
      else if (samplesPerPixel > 20) return waveformLevelsOfDetail.medium;
      else return waveformLevelsOfDetail.high;
    }
  ...
  useEffect(() => {
    if (audioRef.current) {
      const duration = TimelinePosition.fromSpan(
        clipAudio.end.diff(clipAudio.start)
      ).toSeconds();
  ...
  function downsampleWaveformData(
  ...
  function handleDraw(canvas: HTMLCanvasElement | null) {
    if (canvas && audioWidth < WAVEFORM_CHUNK_SIZE) setCopyFrom({ canvas });
  }
  ...
  async function loadAudioData() {
    if (!clipAudio.audioBuffer) {
      ...
      setWaveformLevelsOfDetail({
        ultraLow: downsampleWaveformData(
          clipAudio.audioBuffer,
          Math.floor(data.length / 25000)
        ),
        low: downsampleWaveformData(
          clipAudio.audioBuffer,
          Math.floor(data.length / 12500)
        ),
        medium: downsampleWaveformData(
          clipAudio.audioBuffer,
          Math.floor(data.length / 2500)
        ),
        high: downsampleWaveformData(
          clipAudio.audioBuffer,
          Math.floor(data.length / 200)
        ),
      });
    }
  }
        ...
        if (clipAudio.audioBuffer) {
          ...
          onSetClip({
            ...
            audio: {
              ...
              buffer: audioBufferToBuffer(reversedBuffer),
            },
          });
        }
      },
  ...
  return (
    <>
      <ClipComponent
        automationSprite={(height: number) => (
          <AudioClipWaveform {...wavefromProps(height, true)} />
        )}
        clip={clip}
        height={height}
        listeners={listeners}
        loopSprite={(height: number) => (
          <AudioClipWaveform {...wavefromProps(height, true)} />
        )}
        ...
        sprite={(height: number) => (
          <AudioClipWaveform {...wavefromProps(height, false)} />
        )}
        track={track}
      />
