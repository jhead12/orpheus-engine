...
export default function AutomationLaneComponent({
    ...
    if (lane.nodes && lane.nodes.length === 1) {
      switch (lane.envelope) {
        case AutomationLaneEnvelope.Volume:
          setTrack({
            ...track,
            volume: { ...track.volume, value: lane.nodes[0].value },
          });
          break;
        case AutomationLaneEnvelope.Pan:
          setTrack({
            ...track,
            pan: { ...track.pan, value: lane.nodes[0].value },
          });
          break;
        case AutomationLaneEnvelope.Tempo:
          updateTimelineSettings({
            ...timelineSettings,
            tempo: Math.round(lane.nodes[0].value),
          });
          break;
      }
    }
  ...
  function drawPolylineFromNodes() {
    if (ref.current && polylineRef.current) {
      ...
      if (movingNodeIndex.current > -1 && movingNode.current) {
        const idx = (lane.nodes || []).findIndex(
  ...
  function handleContextMenu(e: React.MouseEvent<HTMLElement>) {
    ...
    openContextMenu(
      ...
      { showPasteOptions: true, disablePaste },
      (params: any) => {
  ...
  function handleNodeMove(node: AutomationNode) {
    if (movingNodeIndex.current === -1 || node.id !== movingNode.current?.id)
      movingNodeIndex.current = (lane.nodes || []).findIndex(
    ...
    while (
      movingNodeIndex.current < (lane.nodes || []).length - 1 &&
  ...
  function setNode(node: AutomationNode) {
    ...
    const laneIndex = automationLanes.findIndex(
      (l: AutomationLane) => l.id === lane.id
    );
