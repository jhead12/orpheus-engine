...
export interface WaveformProps {
  ...
  onDraw?: (
    ...
    auxiliary: HTMLCanvasElement | null
  ) => void;
  width: number;
}
...
export default function Waveform(props: WaveformProps) {
  ...
  useLayoutEffect(() => {
    ...
    return () =>
      timelineEditorWindow.removeEventListener('scroll', handleScroll);
  }, [width, offscreenDrawing]);

  useLayoutEffect(() => {
    drawWaveform(canvasRef.current, chunk);
    if (showAuxiliary) drawWaveform(auxiliaryCanvasRef.current, chunk + 1);
  ...
  function checkChunks(alwaysRenderWaveform = false) {
    ...
    if (timelineEditorWindow && ref.current) {
      ...
      let isInWindow =
        rect.right > timelineEditorWindowRect.left &&
        rect.left < timelineEditorWindowRect.right;

      if ((offscreenDrawing || isInWindow) && ref.current.firstElementChild) {
        const waveformContainerRect =
          ref.current.firstElementChild.getBoundingClientRect();
        isInWindow =
          waveformContainerRect.left < timelineEditorWindowRect.right &&
        ...
        if (offscreenDrawing || isInWindow) {
          ...
          if (width > CHUNK_SIZE) {
            ...
            const scrollPosDistanceFromStart =
              timelineEditorWindowRect.left - waveformContainerRect.left;

            newChunk = Math.floor(scrollPosDistanceFromStart / CHUNK_SIZE);

            if (newChunk < 0) newChunk = 0;
            else if (newChunk >= numChunks) newChunk = numChunks;

            if (
              scrollPosDistanceFromStart + timelineEditorWindow.clientWidth >=
              CHUNK_SIZE * (newChunk + 1)
            )
              newShowAuxiliary = newChunk < numChunks - 1;
            else newShowAuxiliary = false;
          }
      ...
      if (ctx) {
        ...
        for (let i = 0; i < numberOfChannels; i++) {
          ...
          const start = Math.floor(
            ((CHUNK_SIZE * chunk) / width) * channelData.length,
          );
          const end = Math.floor(
            ((CHUNK_SIZE * chunk + canvas.width) / width) * channelData.length,
          );
  ...
  return (
    <div
      className="position-absolute col-12 overflow-hidden"
      ref={ref}
      style={{ top: 0, left: 0, height }}
    >
      <div
        style={{
          position: 'absolute',
          ...
          (chunk + 1) * CHUNK_SIZE < width && (
            <canvas
              ...
              style={{
                position: 'absolute',
                top: 0,
                left: CHUNK_SIZE * (chunk + 1),
              }}
              width={Math.max(
                1,
                Math.min(CHUNK_SIZE, width - CHUNK_SIZE * (chunk + 1)),
              )}
            />
          )}
      </div>
