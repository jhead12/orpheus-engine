...
interface AudioAnalysisPanelProps {
  ...
  clip: Clip | null;
  results?: AudioAnalysisResults | null;
}
...
export default function AudioAnalysisPanel({
  ...
  const renderSpectralAnalysis = (
    ...
    for (let i = 0; i < canvas.width; i++) {
      ...
      const y =
        ((Math.sin(i * 0.05) + 1) * canvas.height) / 4 + canvas.height / 4;
      ctx.lineTo(i, y);
    }
    ...
    if (pythonResults?.features) {
      ...
      if (results?.statistics) {
        ...
        stats.forEach((stat, i) => {
          ctx.fillText(
            stat,
            canvas.width - padding.right - 150,
            padding.top + i * 20
          );
        });
      }
  ...
  return (
    <div className="analysis-panel">
      ...
      {isLoading && <div className="alert alert-info">Analyzing audio...</div>}
