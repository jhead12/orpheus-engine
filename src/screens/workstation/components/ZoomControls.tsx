...
export default function ZoomControls({ onZoom, vertical }: IProps) {
  ...
  const [value, setValue] = useState(
    vertical ? verticalScale : timelineSettings.horizontalScale
  );
  ...
  function horizontalScaleToSliderPos(horizontalScale: number) {
    return (
      0.128144 * Math.log(90.9244 * horizontalScale + 0.946923) - 0.0792586
    );
  }

  function handleChange(_: Event, value: number | number[]) {
    if (vertical) setValue(+lerp((value as number) / 1000, 0.75, 5).toFixed(2));
    else
      setValue(
        +sliderPosToHorizontalScale((value as number) / 1000).toFixed(2)
      );
  }
  ...
  function handleChangeCommitted() {
    ...
    if (vertical) setVerticalScale(value);
    else
  ...
  function zoom(amount: number) {
    ...
    if (vertical) {
      setVerticalScale(
        clamp(+(verticalScale + amount).toPrecision(4), 0.75, 5)
      );
    } else {
      const horizontalScale = clamp(
        timelineSettings.horizontalScale + amount,
        0.01,
        50
      );
  ...
  const sliderValue = useMemo(() => {
    if (vertical) return inverseLerp(value, 0.75, 5) * 1000;
    else return horizontalScaleToSliderPos(value) * 1000;
  }, [vertical, value]);
  ...
  return (
    ...
    >
      ...
      <Slider
        ...
        slotProps={{
          thumb: {
            style: { backgroundColor: 'var(--border6)', width: 8, height: 8 },
          },
