...
function ClipComponent({
  ...
  const [laneTarget, setLaneTarget] = useState<{
    track: Track;
    element: HTMLElement;
  } | null>(null);
  ...
  useEffect(() => {
    if (prevHorizontalScale.current !== undefined) {
      ...
      setCoords({
        startX: coords.startX * percentChange,
        endX: coords.endX * percentChange,
      });
  ...
  useEffect(() => {
    function handleMouseOver(e: MouseEvent) {
      ...
      if (
        ...
        if (
          newEnteredLane &&
          newEnteredLane.dataset.track !== laneTarget?.track.id
        ) {
          const targetTrack = tracks.find(
            (track) => track.id === newEnteredLane.dataset.track
          );
  ...
  useLayoutEffect(() => {
    setHeight(
      laneTarget ? laneTarget.element.children[0].clientHeight + 1 : rest.height
    );
      ...
      waitForScrollWheelStop(timelineEditorWindow, () => {
        if (ref.current) {
          ...
          if (
            startPos >
            timelineEditorWindow.scrollLeft +
              timelineEditorWindow.clientWidth * 0.95 -
              12
          )
            scrollToAndAlign(
            ...
            endPos <
            timelineEditorWindow.scrollLeft +
              timelineEditorWindow.clientWidth * 0.05
          )
            scrollToAndAlign(
              timelineEditorWindow,
              { left: startPos },
          ...
          if (
            ...
            timelineEditorWindowRect.top +
              33 +
              timelineEditorWindow.clientHeight * 0.05
          ) {
            const pos =
              rect.top -
              timelineEditorWindowRect.top +
            ...
            rect.top >
            timelineEditorWindowRect.bottom -
              timelineEditorWindow.clientHeight * 0.05
          ) {
            const pos =
              rect.bottom -
        ...
        switch (params.action) {
          ...
          default:
            for (const listener of listeners || [])
              if (params.action === listener.action) listener.handler();
        }
      });
    }
  }
  ...
  function handleDrag(data: DNRData) {
    ...
    adjustNumMeasures(
      TimelinePosition.fromMargin(Math.max(loopX, data.coords.endX))
    );
    rest.onDrag?.(data);
  }
  ...
  function handleDragStop(e: MouseEvent, data: DNRData) {
    ...
    if (
      laneTarget &&
      laneTarget.track.id !== track.id &&
      laneTarget.track.type === track.type
    )
      onChangeLane(clipAtPos(startPos, clip), laneTarget.track);
    else if (data.delta.x !== 0) onSetClip(clipAtPos(startPos, clip));
  ...
  function handleResize(data: ResizeDNRData) {
    ...
    if (clip.startLimit) {
      ...
      if (newCoords.startX < startLimitX) newCoords.startX = startLimitX;
    }
  ...
  const automationLanes = automationTrack.automationLanes.filter(
    (lane) => lane.show
  );
  ...
  return (
    <>
      <DNR
        ...
        resizeHandles={{
          left: { style: { zIndex: selected ? 15 : 14 } },
          right: { style: style.resizeRight },
        }}
      ...
      >
        <div className="position-relative m-0" style={style.clipNameContainer}>
          <div
            ...
            style={style.clipNameContainerInner}
          >
            {renaming ? (
              <form
                onSubmit={(e) => {
        ...
        <div
          ...
          style={style.spriteContainer}
        >
          <div style={{ ...style.clip, ...style.sprite }}>
            {rest.sprite?.(height - 16)}
          </div>
        </div>
        {loopWidth > 5 && (
          <div className="position-absolute h-100" style={style.loopOverlay} />
        )}
        <div
          className="position-absolute pe-none"
          style={style.automationContainer}
        >
            ...
            automationLanes.map((lane: AutomationLane, idx: number) => {
              ...
              return (
                <div
                  className="d-flex"
                  key={idx}
                  style={style.automationLaneContainer(lane, idx)}
                >
                  {width > 0 ? (
                    Array.from(
                      { length: Math.ceil(repetitions) + 1 },
                      (_, i) => (
                        <div
                          className="position-relative"
                          key={i}
                          style={style.automationLoopContainer(i)}
                        >
                          <div
                            style={{
                              height: "100%",
      ...
      >
        <div
          ...
          style={style.loopContainer}
        >
          {width > 0 &&
            Array.from({ length: Math.ceil(repetitions) }, (_, i) => (
              <div key={i} style={{ ...style.clip, ...style.loopRepetition }}>
                <div style={{ opacity: 0.6 }}>
                  {rest.loopSprite?.(height - 16)}
                </div>
              </div>
            ))}
        </div>
