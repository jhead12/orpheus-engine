...
const MixerTrack = memo(
  ({ order, track }: { order?: number; track: Track }) => {
    ...
    const ensureAutomatableParameter = (
      value: number | AutomatableParameter | undefined | null,
    ): AutomatableParameter => {
    ...
    return (
      <div
        className={`col-auto mixer-track pr-2 pl-2 border-right border-dark-1${
          selected ? ' overlay-1' : ''
        }`}
        data-testid={
          isMaster ? 'mixer-master-channel' : `mixer-channel-${track.id}`
        }
      ...
      >
        <div className="col-12 m-0 p-0">
            ...
            <div className="col-4 m-0 px-1">
              <div style={{ display: 'flex' }}>
                <div style={{ marginRight: 1 }}>
                  <Meter
                    color={getVolumeGradient(
                      ensureAutomatableParameter(track.volume)?.value ?? 0,
                    )}
                    percent={
                      volumeToNormalized(
                        ensureAutomatableParameter(track.volume)?.value ?? 0,
              ...
              <div
                ...
                data-testid={
                  isMaster
                    ...
                    : `mixer-volume-display-track-${track.id}`
                }
                style={{ fontSize: 10, color: 'var(--border6)' }}
              >
                {Math.round(
                  (ensureAutomatableParameter(track.volume)?.value ?? 0) * 100,
                )}
                %
              </div>
            </div>
                ...
                <div className="row m-0">
                  ...
                  <div className="col-4 m-0 p-0">
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                      ...
                      >
                        {formatPanning(
                          ensureAutomatableParameter(pan)?.value ?? 0,
                          true,
                        )}
                      </span>
                    </div>
                    ...
                    {!isMaster && (
                      <div>
                        ...
                        <IconButton
                          ...
                          style={{
                            color: track.armed ? '#ff004c' : 'var(--border6)',
                          }}
                          title={
          ...
          <div
            style={style.peakLevel}
            data-testid={`mixer-peak-track-${track.id}`}
          >
        ...
        <div className="col-12 m-0 p-0">
          {isMaster ? (
            <div className="text-center py-2" style={style.masterText}>
              MASTER
            </div>
          ) : (
            <div className="col-12 m-0 p-0" style={style.trackTextContainer}>
              ...
              <div className="text-center" style={style.orderTextContainer}>
      ...
      if (e.key === 'ArrowRight' || e.key === 'ArrowLeft') {
        ...
        if (currentTestId?.startsWith('mixer-channel-')) {
          ...
          if (currentIndex !== -1) {
            ...
            if (e.key === 'ArrowRight') {
              nextIndex =
                currentIndex + 1 < tracks.length ? currentIndex + 1 : 0;
            } else {
              nextIndex =
                currentIndex - 1 >= 0 ? currentIndex - 1 : tracks.length - 1;
            }

            const nextTrackId = tracks[nextIndex].id;
            const nextElement = document.querySelector(
              `[data-testid="mixer-channel-${nextTrackId}"]`,
            ) as HTMLElement;
            nextElement?.focus();
          }
        }
