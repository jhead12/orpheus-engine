...
> {
  ...
  handleResizeStart(edge: 'start' | 'end') {
    if (!this.props.onResizeStart || !this.ref.current || this.state.resizing)
      return;

    this.setState({
      resizing: true,
      resizeEdge: edge,
      temp: this.state.region,
    });
  ...
  resize(x: number, edge: 'start' | 'end') {
    ...
    const { horizontalScale = 1, timeSignature = { noteValue: 4 } } =
      this.context.timelineSettings;
    const snapWidth =
    ...
    if (edge === 'start') {
      ...
      const snappedMargin = snapWidth
        ? snapWidth * Math.round(newPos.toMargin() / snapWidth)
        : newPos.toMargin();
      region = {
        ...temp,
    ...
    return (
      <>
        <WindowAutoScroll
          ...
          onScroll={(by: number) =>
            this.resize(by, this.state.resizeEdge || 'end')
          }
        />
        <div
          ...
          style={{
            cursor:
              !this.state.isCreatingNewRegion && !this.state.resizing
                ? 'default'
                : '',
            height: '100%',
            left: startMargin,
