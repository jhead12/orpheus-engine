...
export default function FXComponent({
  ...
  useEffect(() => {
    if (track.fx.effects.length > 0 && effectIndex >= track.fx.effects.length)
      setTrack({
        ...track,
        fx: { ...track.fx, selectedEffectIndex: track.fx.effects.length - 1 },
      });
  }, [track.fx.effects]);

  useLayoutEffect(() => {
    presetNameInputRef.current?.setSelectionRange(
      0,
      presetNameInputRef.current?.value.length,
    );
  ...
  function addEffect() {
    ...
    setTrack({
      ...track,
      fx: { ...track.fx, effects, selectedEffectIndex: effects.length - 1 },
    });
  }

  function deleteEffect() {
    const effects = track.fx.effects.filter(
      (_, idx: number) => idx !== effectIndex,
    );
    setTrack({ ...track, fx: { ...track.fx, effects } });
  }

  function handleBlur(e: React.FocusEvent<HTMLFormElement>) {
    if (!e.currentTarget.contains(e.relatedTarget)) hideNamePresetMode();
  }

  function more() {
    if (preset) {
      openContextMenu(
        ContextMenuType.FXChainPreset,
        { presetModified },
        (params: any) => {
          switch (params.action) {
            ...
            case 1:
              ...
              break;
            case 2:
              const selectedEffectIndex = Math.min(
                effectIndex,
                preset.effects.length - 1,
              );
              setTrack({
                ...track,
                fx: {
            ...
            case 3:
              ...
              for (let i = 0; i < newTracks.length; i++)
                if (newTracks[i].fx.preset === preset?.id)
                  newTracks[i] = {
                    ...newTracks[i],
                    fx: { ...newTracks[i].fx, preset: null },
                  };

              if (masterTrack.fx.preset === preset?.id)
                setTrack({
                  ...masterTrack,
                  fx: { ...masterTrack.fx, preset: null },
                });

              setFXChainPresets(
                fxChainPresets.filter((p) => p.id !== preset.id),
              );
              setTracks(newTracks);
              break;
          }
  ...
  function onChangePreset(value: string | number) {
    const newPreset = fxChainPresets.find(
      (preset) => preset.id === value.toString(),
    );
    ...
    if (preset?.id !== newPreset?.id) {
      ...
      setTrack({
        ...track,
        fx: { preset: presetId, effects, selectedEffectIndex: 0 },
      });
    }
  }
    ...
    if (newName) {
      if (saveAsNew) {
        const newPreset = {
          id: v4(),
          name: newName,
          effects: track.fx.effects.slice(),
        };
  ...
  function spin(direction: string) {
    if (direction === 'prev' && effectIndex > 0)
      setTrack({
        ...track,
        fx: { ...track.fx, selectedEffectIndex: effectIndex - 1 },
      });
  ...
  function selectExternalSource() {
    ...
    if (effect?.type === 'juce' || effect?.type === 'python') {
      ...
      filePicker.accept =
        effect.type === 'juce' ? '.vst3,.au,.dll,.component' : '.py';
  ...
  const disableSave =
    (preset && !presetModified) || track.fx.effects.length === 0;
  ...
  const style = {
    ...
    toggle: {
      button: {
        ...
        padding: 0,
        marginLeft: 3,
        ...rest.style?.toggle?.button,
      },
      icon: {
        fontSize: 14,
        opacity: showPreset ? 1 : 0.4,
        ...rest.style?.icon,
        ...rest.style?.toggle?.icon,
      },
    },
    top: {
      height: 19,
  ...
  return (
    <div
      className={`d-flex col-12 stop-reorder ${classes?.container}`}
      ref={ref}
    ...
    >
      ...
      {(!compact || showPreset) && (
        <div
          className={`d-flex align-items-center overflow-hidden ${classes?.top}`}
          style={style.top}
        >
          {namePreset ? (
            <form
            ...
            <>
              <SelectSpinBox
                ...
                title={
                  preset
                    ? `${preset.name}${presetModified ? ' (edited)' : ''}`
                    : ''
                }
              ...
              >
                <IconButton
                  className={`p-0 ${disableSave ? 'disabled' : ''}`}
                  onClick={save}
                  title="Save"
                >
      ...
      {(!compact || !showPreset) && (
        <div
          className={`d-flex w-100 overflow-hidden ${classes?.bottom}`}
          style={style.bottom}
        >
          ...
          <div
            ...
            style={{
              ...
              paddingLeft: 2,
              cursor: 'pointer',
              ...rest.style?.effect?.container,
            }}
            tabIndex={0}
            data-testid={
              effect
                ? `${effectTestIdPrefix || 'effect'}-${effect.id}`
                : undefined
            }
          >
            {effect && (
              <React.Fragment>
                ...
                >
                  {effect.name}
                  {effect.type !== 'native' && (
                    <small style={{ opacity: 0.7 }}> ({effect.type})</small>
                  )}
                </p>
                  ...
                  <div className="d-flex align-items-center mr-2">
                    <SelectSpinBox
                      ...
                      onChange={(value: string) =>
                        changeEffectType(value as 'native' | 'juce' | 'python')
                      }
                      options={FX_TYPES}
                      style={{
                ...
                >
                  ...
                  >
                    <Delete
                      className={`p-0 ${classes?.removeIcon}`}
          ...
          {track.fx.effects.length > 0 && (
            <div
              className={classes?.spinButtons}
              style={{ width: 16, ...rest.style?.spinButtons }}
            >
              <button
                className={`col-12 overflow-hidden center-flex focus-3 ${classes?.prev?.button}`}
                onClick={() => spin('prev')}
                style={{
                  ...style.spinButton(effectIndex === 0),
                  ...rest.style?.prev?.button,
                }}
              ...
              <button
                ...
                onClick={() => spin('next')}
                style={{
                  ...style.spinButton(lastEffect),
                  ...rest.style?.next?.button,
                }}
