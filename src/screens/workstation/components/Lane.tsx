...
const Lane = forwardRef<HTMLDivElement, LaneComponentProps>((props, ref) => {
  ...
  useEffect(() => {
    if (selectedTrackId === track.id)
      if (trackRegion && trackRegion.trackId !== track.id) setTrackRegion(null);
  ...
  useEffect(() => {
    ...
    function handleDragOver(e: DragEvent) {
      ...
      if (internalRef.current) {
        let margin =
          e.clientX - internalRef.current.getBoundingClientRect().left;
        ...
        if (margin !== prevMargin.current) {
          ...
          margin = snapWidth
            ? snapWidth * Math.round(margin / snapWidth)
            : margin;
  ...
  function onLaneContextMenu(e: React.MouseEvent<HTMLElement>) {
    ...
    const disablePaste =
      clipboardData?.type !== ClipboardItemType.Clip ||
      clipboardData.item.type !== track.type;

    openContextMenu(
      ContextMenuType.Lane,
      { track, disablePaste },
      (params: any) => {
        switch (params.action) {
          case 0:
            window.electronAPI.ipcRenderer
              .invoke(TRACK_FILE_UPLOAD, track.type)
  ...
  if (showMaster || !isMaster) {
    return (
      ...
      >
        ...
        >
          {track.type !== TrackType.Audio && (
            <RegionComponent
              autoScroll={{
                thresholds: {
                  left: timelineEditorWindowScrollThresholds[0],
                  right: timelineEditorWindowScrollThresholds[1],
                },
              }}
              onContextMenu={(e: React.MouseEvent<HTMLDivElement>) =>
                onTrackRegionContextMenu(e)
              }
              onSetRegion={(region: any) =>
                setTrackRegion(region ? { region, trackId: track.id } : null)
              }
        ...
        {isDragTarget && !dragDataTarget?.incompatible && (
