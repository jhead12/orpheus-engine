...
export function formatDuration(
  ...
  const m = duration.minutes
    .toString()
    .padStart(verbose || duration.hours !== 0 ? 2 : 0, "0");
  const s = duration.seconds.toString().padStart(2, "0");
  const ms = Math.round(duration.milliseconds)
    .toString()
    .padStart(3, "0")
    .slice(0, 2);
...
export function parseDuration(timeStr: string) {
  ...
  while (arr.length > 0) {
    ...
    if (timeUnitInSeconds > 3600) return null;
    if ((!part && arr.length > 0) || isNaN(Number(part))) return null;
...
export function shadeColor(hex: string, amt: number) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    channel = Math.round(
      Math.min(Math.max(0, channel + channel * amt), 255)
    ).toString(16);
    newHex += channel;
  }
...
export function truncate(num: number, places: number = 0) {
  if (num < 0) return (Math.ceil(num * 10 ** places) / 10 ** places).toString();
  else return (Math.trunc(num * 10 ** places) / 10 ** places).toString();
}
