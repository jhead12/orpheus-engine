...
function updateWebpackConfig() {
  const possibleConfigLocations = [
    '/workspaces/orpheus-engine/webpack.config.js',
    '/workspaces/orpheus-engine/OEW-main/webpack.config.js',
    '/workspaces/orpheus-engine/orpheus-engine-workstation/webpack.config.js',
  ];
...
function modifyWebpackConfig(configPath) {
  ...
  if (content.includes('proxy:') && content.includes('http://localhost:8001')) {
    const updatedContent = content.replace(
      ...
      "            res.end(JSON.stringify(" + JSON.stringify(defaultManifest) + "));\n" +
      "          } else {\n" +
      "            res.writeHead(502);\n" +
...
async function main() {
  ...
  if (!localManifestFound) {
    ...
    for (const dir of publicDirs) {
      if (fs.existsSync(dir)) {
        fs.writeFileSync(path.join(dir, 'manifest.json'), JSON.stringify(defaultManifest, null, 2));
  ...
  console.log('Proxy fallback setup complete. The application will now serve a default manifest.json when needed.');
  console.log('If you want to properly fix this error, please ensure your service on port 8001 is running.');
}
