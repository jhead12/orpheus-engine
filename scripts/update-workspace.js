...
/**
 * Script to update workspace references from OEW-main to workstation
 * and fix Google Maps related dependency issues
 */
...
console.log(`${colors.cyan}Updating workspace references and fixing dependency issues...${colors.reset}`);
...
function updateMainPackageJson() {
  ...
  try {
    console.log(`${colors.yellow}Reading ${packageJsonPath}...${colors.reset}`);
    ...
    // Update start:daw:local script to use workstation instead of OEW-main
    if (packageJson.scripts && packageJson.scripts['start:daw:local']) {
      if (packageJson.scripts['start:daw:local'].includes('cd OEW-main')) {
        console.log(`${colors.yellow}Updating start:daw:local script...${colors.reset}`);
        packageJson.scripts['start:daw:local'] = packageJson.scripts['start:daw:local'].replace('cd OEW-main', 'cd workstation/frontend');
        console.log(`${colors.green}Updated start:daw:local script to use workstation/frontend${colors.reset}`);
      } else {
        console.log(`${colors.green}start:daw:local script is already updated${colors.reset}`);
      }
    }
    
    // Add a fix-maps-dependency script
    if (!packageJson.scripts['fix-maps-dependency']) {
      console.log(`${colors.yellow}Adding fix-maps-dependency script...${colors.reset}`);
      packageJson.scripts['fix-maps-dependency'] = 'node scripts/fix-maps-dependency.js';
      console.log(`${colors.green}Added fix-maps-dependency script${colors.reset}`);
    }
    
    // Write the updated package.json
    fs.writeFileSync(packageJsonPath, JSON.stringify(packageJson, null, 2));
    console.log(`${colors.green}Updated ${packageJsonPath}${colors.reset}`);
    return true;
  } catch (error) {
...
// Create a script to fix Google Maps dependencies
function createMapsFixScript() {
  const scriptPath = path.join(process.cwd(), 'scripts', 'fix-maps-dependency.js');
...
`;
  ...
  try {
    ...
    console.log(`${colors.green}Created Google Maps fix script at ${scriptPath}${colors.reset}`);
    return true;
  } catch (error) {
    console.error(`${colors.red}Error creating fix script: ${error.message}${colors.reset}`);
    return false;
  }
}

// Run the update functions
const packageJsonUpdated = updateMainPackageJson();
const fixScriptCreated = createMapsFixScript();

if (packageJsonUpdated && fixScriptCreated) {
  console.log(`${colors.green}✓ Successfully updated workspace references and created fix scripts${colors.reset}`);
  console.log(`${colors.yellow}To fix Google Maps dependencies, run:${colors.reset}`);
  console.log(`${colors.cyan}npm run fix-maps-dependency${colors.reset}`);
} else {
  console.log(`${colors.yellow}Some updates were not completed successfully${colors.reset}`);
}
